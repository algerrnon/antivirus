...
Список файлов, которые будут подвергнуты очистке от угроз 
содержится в настройке 
__avApi.fileExtensionsLists.forThreadExtraction__

Список файлов, которые должны быть конвертированы в PDF 
содержится в настройке
__avApi.fileExtensionsLists.forConvertToPdf__

Часть конфигурационного файла с установленными настройками:
```
avApi {
  fileExtensionsLists = {
    forThreadExtraction = ["doc", "docx", "xls", "xlsx", "png", "jpeg"],
    forConvertToPdf = ["doc", "docx"]
  } 
```
...
|Настройка|Значение|Пример|
|---|---|---|
|customNotice.pleaseWait|Сообщение отправляемое в чат через Chat API Version 2 with CometD. Сообщение отправляется после загрузки файла во временный каталог, но до проверки файла антивирусной системой и до отправки проверенного файла в чат конечному получателю|Пожалуйста подождите происходит антивирусная проверка файла|
|customNotice.isSafeFile|Вам будет отправлен безопасный файл|Файл безопасен, не содержит угроз и будет отправлен вам|
|customNotice.isSafeFileAndLinkToOriginal|Вам будет отправлен безопасный файл. Также в сообщении будет ссылка, при помощи которой вы можете запросить оригинальный файл|Файл безопасен и не содержит угроз. Оригинальная версия файла доступна по ссылке: |
|customNotice.isInfectedFile|Этот файл опасен|Файл содержит угрозы. Использование этого файла запрещено|
|customNotice.isCorruptedFile|Это файл поврежден, зашифрован или содержит ошибки|Файл поврежден, содержит ошибки или защищен паролем|
|customNotice.fileNotFound|Файл отсутствует и не может быть получен|Файл отсутствует или не может быть получен|
