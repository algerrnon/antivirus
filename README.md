...
При отправке запроса к __Chat API__ Version 2 with CometD используются clientId, secureKey и заголовок cookie, эти значения извлекаются из первоначального запроса __Клиента__ на загрузку файла. 

Текст отправляемого сообщения __message__ получаем настроек хранящихся в разделе __customNotices__ файла application.conf , например  это может быть __customNotices.pleaseWait__ 
