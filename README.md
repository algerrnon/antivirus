...
#### Отправка сообщений в чат (Сustom Notice) при помощи Genesys Chat API  

Во время всего сценария обработки, для уведомления __Пользователя__ о происходящих событиях,
__Сервис__ использует возможность отправки сообщений в чат.
Это могут быть сообщения о том, что файл проходит проверку или, что выслала безопасная копия файла или другие.
Перечень всех отправляемых сообщений вы можете увидеть в таблице [Сообщения отправляемые пользователю](#сообщения-отправляемые-пользователю)

При необходимости отправить уведомление в чат
__Система__ отправляет запрос к [Genesys Chat API Version 2 with CometD](https://docs.genesys.com/Documentation/GMS/8.5.2/API/ChatAPIv2CometD)

В запросе вызывается операция __customNotice__

При отправке запроса к __Chat API__ Version 2 with CometD используются значения clientId, secureKey и заголовок cookie, они извлекаются ранее из первоначального запроса __Клиента__ на загрузку файла. 

Текст отправляемого сообщения __message__ получаем из настроек хранящихся в разделе __customNotices__ файла application.conf , например  это может быть __customNotices.pleaseWait__ 
